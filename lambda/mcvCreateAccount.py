...
def validateJSON(requestBody):
    if requestBody is not None:
        ...
        try:
            ...
            if "password" not in requestJsonObject:
                return False, "'password' missing in Request Body", None
            if not re.fullmatch(regexPassword, requestJsonObject["password"]):
                return False, "Value of 'password' must have 8 characters with at least 1 uppercase, 1 lowercase, 1 number and 1 special character, which can be any of: (.*?[#?!@$%^&*-])", None
...
def lambda_handler(event, context):
    ...
    if event['httpMethod'] != 'POST':
        ...
        responseDictBarebones['headers']['Access-Control-Expose-Headers'] = "Content-Type"
        responseDictBarebones['headers']['Content-Type'] = "application/json"
        responseDictBarebones['body'] = json.dumps('Only POST method allowed for Account Creation')
        return responseDictBarebones
    ...
    #=========================================================
        ...
        if not isRequestBodyValid:
            ...
            password = requestJsonObject["password"]
            
            #=========================================================
            #WRITE TO DYNAMODB TABLE mycatvideos_users
            #=========================================================
            try:
                dynamodb.Table('mycatvideos_users').put_item(
                    Item={
                        'userid': userid,
                        'password': password,
                        'loggedin': '0'
                ...
                if e.response['Error']['Code'] == 'ConditionalCheckFailedException':
                    ...
                    errorMessage = "Error writing to dynamodb. Details: " + str(e)
                responseDictBarebones['statusCode'] = 400
                responseDictBarebones['headers']['Access-Control-Expose-Headers'] = "Content-Type"
                responseDictBarebones['headers']['Content-Type'] = "application/json"
                responseDictBarebones['body'] = json.dumps(errorMessage)
